...
export const StripeScreen: React.FC<Props> = ({ route, navigation }) => {
  
    const handleChange = (e: WebViewNavigation): void => {
    if (
      !e.loading &&
      e.url === `${getEnvVariables().successPaymentUrl}`
    ) {
  ...
  return (
    <WebView
      source={{
        uri: `${getEnvVariables().generateStripeUrl}?sessionId=${route.params.sessionId}`,
      }}
